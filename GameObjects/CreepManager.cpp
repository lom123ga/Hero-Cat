...
void CreepManager::SpawnRino()
{
	...
	creep->setStartPoint(sf::Vector2f(screenWidth + rand() % (150 - 50 + 1) + 50, START));
...
void CreepManager::SpawnBat()
{
	...
	if (creep == nullptr) return;
	creep->getHitBox()->setAlive(true);
	creep->setStartPoint(sf::Vector2f(screenWidth + rand() % (150 - 50 + 1) + 50, START - (rand() % (200 - 100 + 1) + 100)));
	creep->Reset();
}
